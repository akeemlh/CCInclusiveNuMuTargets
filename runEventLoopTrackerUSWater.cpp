#define MC_OUT_FILE_NAME "runEventLoopTrackerUSWaterMC.root"
#define DATA_OUT_FILE_NAME "runEventLoopTrackerUSWaterData.root"
      ...
      for (auto universe : error_band_universes)
      {
        ...
        if(isSignal)
        {
          ...
          for(auto& var: vars2D)
          {
            ...
            var->migration->Fill(var->GetRecoValueX(*universe), var->GetRecoValueY(*universe), var->GetTrueValueX(*universe), var->GetTrueValueY(*universe),universe->ShortName(),(univCount-1),weight);
            //var->FillMigration(universe, var->GetTrueValueX(*universe), var->GetTrueValueY(*universe), var->GetRecoValueX(*universe), var->GetRecoValueY(*universe), weight);
          }
        }
        else
        {
...
int main(const int argc, const char** argv)
{
  ...
  preCuts.emplace_back(new reco::ZRange<CVUniverse, MichelEvent>("Active Tracker Z pos", PlotUtils::TargetProp::NukeRegion::Face, PlotUtils::TargetProp::WaterTarget::Face));
  ...
  phaseSpace.emplace_back(new truth::ZRange<CVUniverse>("Active Tracker Z pos", PlotUtils::TargetProp::NukeRegion::Face, PlotUtils::TargetProp::WaterTarget::Face));
