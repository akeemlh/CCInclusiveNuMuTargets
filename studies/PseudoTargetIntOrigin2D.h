...
class PseudoTargetIntOrigin2D: public Study
{
  public:
    using reco_t = std::function<double(const CVUniverse&, const MichelEvent&)>;

    PseudoTargetIntOrigin2D(reco_t reco_x, reco_t reco_y, const std::string& varName, const std::string& varUnits, const std::vector<double> xBins, const std::vector<double> yBins, const std::map<std::string, std::vector<CVUniverse*>>& univs): Study(), fReco_x(reco_x), fReco_y(reco_y)
    {
