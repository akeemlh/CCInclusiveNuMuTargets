  ...
  if (files) {
      ...
      while ((file=(TSystemFile*)next())) {
          ...
          if (!file->IsDirectory()) {
              ...
              if(fname.EndsWith("TrackerData.root")) //Just this once while I'm using combined for testing
              {
              ...
              else if(fname.EndsWith("TrackerMC.root")) //Just this once while I'm using combined for testings
              {
...
int main(const int argc, const char** argv)
{
  ...
  std::string dataFilePath = std::string(argv[2])+"/runEventLoopTrackerData.root";
  ...
  std::string mcFilePath = std::string(argv[2])+"/runEventLoopTrackerMC.root";
  ...
  for(const auto& prefix: crossSectionPrefixes)
  {
    try
    {
      ...
      */
      ...
      backgrounds.push_back(util::GetIngredient<PlotUtils::MnvH1D>(*mcFile, "background_NC", prefix));
      backgrounds.push_back(util::GetIngredient<PlotUtils::MnvH1D>(*mcFile, "background_Wrong_Sign", prefix));
      ...
      auto outFile = TFile::Open((prefix+std::string("_tracker_crossSection.root")).c_str(), "RECREATE");
      if(!outFile)
      {
        std::cerr << "Could not create a file called " << prefix <<"_tracker_crossSection.root" << ".  Does it already exist?\n";
        return 5;
      }
