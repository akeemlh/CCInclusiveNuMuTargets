    ...
    for attempt in range(max_retries):
        ...
        try:
            ...
            # If the command failed
                ...
                print(result.stderr.strip() or "No standard error output captured.")
                print("--------------")
...
if __name__ == '__main__':
    ...
    for playlist in vettedPlaylists:
        ...
        if numOfSubRuns != 0:
            subrunstr = "export NumGridSubruns="+str(numOfSubRuns)+"\n"
            my_wrapper.write(subrunstr)
        if specifySubrun!= None:
            subrunstr = "export NumGridSubruns="+str(specifySubrunTotal)+"\n"
            my_wrapper.write(subrunstr)
            subrunstr = "export PROCESS="+str(specifySubrun)+"\n"
            my_wrapper.write(subrunstr)
        ...
        my_wrapper.write("    ${MINERVA_PREFIX}/bin/runEventLoopValidationsNew ${CONDOR_DIR_INPUT}/"+playlist+"-Data.txt "+"${CONDOR_DIR_INPUT}/"+playlist+"-MC.txt || exit $?\n") #The "|| exit $?" is so that if this command fails the bash script will also exit which will trigger the condor job retry
        ...
        my_wrapper.write("ifdh cp ./VertexValidations* "+outdirplaylist+"\n")
        ...
        print(cmd)
        success = submit_with_retries(cmd, 5, 60)
        #os.system(cmd)
