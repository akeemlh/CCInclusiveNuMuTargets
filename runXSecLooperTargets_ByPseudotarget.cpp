...
public:
  ...
  {
    //if ((int)chw.GetValue("truth_targetZ", entry)!=0) std::cout<<"truth_targetZ: " << (int)chw.GetValue("truth_targetZ", entry) << "\t entry: " << entry << std::endl;
    //if((int)chw.GetValue("truth_targetZ", entry)!=fTargetZ) return false;
    ...
    /* std::cout << " nucleus " << nucleus <<std::endl;
    std::cout << " target " << target <<std::endl;
    std::cout << " fTargetCode " << fTargetCode <<std::endl;
    std::cout << " truth_targetZ " << (int)chw.GetValue("truth_targetZ", entry) <<std::endl;
    std::cout << " truth_target_code " << (int)chw.GetValue("truth_target_code", entry) <<std::endl; */
    //std::cout<<"Here111\n";
    int true_module   = (int)chw.GetValue("truth_vtx_module",entry);

    if ((fTargetCode == 7) && (true_module<-5 || true_module>-2)) return false;
    if ((fTargetCode == 8) && (true_module<0 || true_module>3)) return false;
    if ((fTargetCode == 9) && (true_module<5 || true_module>8)) return false;
    if ((fTargetCode == 10) && (true_module<11 || true_module>14)) return false;
    if ((fTargetCode == 11) && (true_module<15 || true_module>18)) return false;
    if ((fTargetCode == 12) && (true_module<20 || true_module>21)) return false;
    std::cout<<"true_module " << true_module <<std::endl;
...
double GetNormValue( int targetID )
{
...
int main(const int argc, const char** argv)
{
  ...
  std::vector<int> targetCodes = {7, 8, 9, 10, 11, 12};
  ...
  /* 
    ...
    */
    ...
    // Flux-integrated over the range 0.0 to 120.0 GeV
