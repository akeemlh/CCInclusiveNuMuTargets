...
void Plot(PlotUtils::MnvH1D& hist, const std::string& stepName, const std::string& prefix)
{
  ...
  can.Print(("Tracker_"+prefix + "_" + stepName + ".png").c_str());
  ...
  can.Print(("Tracker_"+prefix + "_" + stepName + "_uncertaintySummary.png").c_str());

  plotter.DrawErrorSummary(&hist, "TR", true, true, 1e-5, false, "Other");
  can.Print(("Tracker_"+prefix + "_" + stepName + "_otherUncertainties.png").c_str());
}
...
std::pair<double, double> getCombinedPOTs(std::string dir, std::vector<std::string> playlists)
{
  ...
  {
    std::string datafilepath = dir+p+"-runEventLoopDataTracker.root";
    std::string mcfilepath = dir+p+"-runEventLoopMCTracker.root";
...
int main(const int argc, const char** argv)
{
  ...
  {
    std::string datafilepath = dir+std::string(argv[i])+"-runEventLoopDataTracker.root";
    ...
    dataFile->Close();
    std::string mcfilepath = dir+std::string(argv[i])+"-runEventLoopMCTracker.root";
  ...
  std::string dataFilePath = dir+std::string(playlists[0])+"-runEventLoopDataTracker.root";
  auto dataFile = TFile::Open(dataFilePath.c_str(), "READ");
  ...
  {
    std::string outFilePath = prefix + "_Tracker_crossSection.root";
    auto outFile = TFile::Open(outFilePath.c_str(), "RECREATE");
    if(!outFile)
    {
    ...
    {
      std::cout << "Saving to " << prefix + "_Tracker_crossSection.root" <<std::endl;
    }
    ...
    PlotUtils::MnvH1D* bkgSubtracted;
    PlotUtils::MnvH1D* unfolded;
    PlotUtils::MnvH1D* toSubtract;
    PlotUtils::MnvH1D* crossSection;
    try
    {
      for (auto playlist : playlists)
      {
        std::string dataFilePath = dir + std::string(playlist)+"-runEventLoopDataTracker.root";
        dataFile = TFile::Open(dataFilePath.c_str(), "READ");

        std::string mcFilePath = dir + std::string(playlist)+"-runEventLoopMCTracker.root";
        auto mcFile = TFile::Open(mcFilePath.c_str(), "READ");

        dataPOT += util::GetIngredient<TParameter<double>>(*dataFile, "POTUsed")->GetVal(); //relabel as combined POT to be less ambiguous
        mcPOT += util::GetIngredient<TParameter<double>>(*mcFile, "POTUsed")->GetVal();

        std::cout<<"Here1\n";
        if (flux==nullptr) flux = util::GetIngredient<PlotUtils::MnvH1D>(*mcFile, "reweightedflux_integrated", prefix);
        else flux->Add(util::GetIngredient<PlotUtils::MnvH1D>(*mcFile, "reweightedflux_integrated", prefix));
        if (folded==nullptr) folded = util::GetIngredient<PlotUtils::MnvH1D>(*dataFile,  "data", prefix);
        else folded->Add(util::GetIngredient<PlotUtils::MnvH1D>(*dataFile,  "data", prefix));
        if (migration==nullptr) migration = util::GetIngredient<PlotUtils::MnvH2D>(*mcFile, "migration", prefix);
        else migration->Add(util::GetIngredient<PlotUtils::MnvH2D>(*mcFile, "migration", prefix));
        if (effNum==nullptr) effNum = util::GetIngredient<PlotUtils::MnvH1D>(*mcFile, "efficiency_numerator", prefix);
        else effNum->Add(util::GetIngredient<PlotUtils::MnvH1D>(*mcFile, "efficiency_numerator", prefix));
        if (effDenom==nullptr) effDenom = util::GetIngredient<PlotUtils::MnvH1D>(*mcFile, "efficiency_denominator", prefix);
        else effDenom->Add(util::GetIngredient<PlotUtils::MnvH1D>(*mcFile, "efficiency_denominator", prefix));
        ...
        */
        ...
        nNucleons = util::GetIngredient<TParameter<double>>(*mcFile, "fiducial_nucleons", prefix); //Dan: Use the same truth fiducial volume for all extractions.  The acceptance correction corrects data back to this fiducial even if the reco fiducial cut is different.
        ...
        if (backgrounds.find("NC") == backgrounds.end()) {
          backgrounds["NC"]=util::GetIngredient<PlotUtils::MnvH1D>(*mcFile, "background_NC", prefix);
        } else {
          backgrounds["NC"]->Add(util::GetIngredient<PlotUtils::MnvH1D>(*mcFile, "background_NC", prefix));
        }
        //if (backgrounds.find("WrongMaterial") == backgrounds.end()) {
        //  backgrounds["WrongMaterial"]=util::GetIngredient<PlotUtils::MnvH1D>(*mcFile, "background_Wrong_Material", prefix);
        //} else {
        //  backgrounds["WrongMaterial"]->Add(util::GetIngredient<PlotUtils::MnvH1D>(*mcFile, "background_Wrong_Material", prefix));
        //}
        if (backgrounds.find("WrongSign") == backgrounds.end()) {
          backgrounds["WrongSign"]=util::GetIngredient<PlotUtils::MnvH1D>(*mcFile, "background_Wrong_Sign", prefix);
        } else {
          backgrounds["WrongSign"]->Add(util::GetIngredient<PlotUtils::MnvH1D>(*mcFile, "background_Wrong_Sign", prefix));
        }
      ...
      gSystem->cd(outFilePath.c_str());
      gSystem->ChangeDirectory(outFilePath.c_str());
      ...
      toSubtract = std::accumulate(std::next(backgrounds.begin()), backgrounds.end(), (*backgrounds.begin()).second->Clone(),
      ...
      bkgSubtracted = std::accumulate(backgrounds.begin(), backgrounds.end(), folded->Clone(),
      ...
      bkgSubtracted->SetDirectory(outFile);

      std::string outFilePath = prefix + "_Tracker_crossSection.root";
      auto test = TFile::Open(outFilePath.c_str(), "RECREATE");
      if(!test)
      {
        std::cerr << "Could not create a file called " << prefix + "_crossSection.root" << ".  Does it already exist?\n";
        return 5;
      ...
      unfolded = UnfoldHist(bkgSubtracted, migration, nIterations);
      ...
      crossSection = normalize(unfolded, flux, nNucleons->GetVal(), dataPOT);
      std::cout <<"Print reached here bro!\n";
      crossSection->SetDirectory(outFile);
    ...
    std::cout<<"HHERRE1\n";
    simEventRate->SetDirectory(outFile);
    TDirectory* dir2 = simEventRate->GetDirectory();
    std::cout<<"HHERRE2\n";
    const char * path2 = dir2->GetPath();
    std::cout<<"Path: " << path2 << "\n";
    std::cout << "Directory to save root file: " << path2 << std::endl;
    Plot(*simEventRate, "test", prefix);
    std::cout<<"HHERRE3\n";
    simEventRate->SetDirectory(outFile);
    std::cout<<"HHERRE4\n";
    simEventRate->Write();
    std::cout<<"HHERRE5\n";
    toSubtract->SetDirectory(outFile);
    std::cout<<"HHERRE6\n";
    toSubtract->Write("Test");
    std::cout<<"HHERRE7\n";
