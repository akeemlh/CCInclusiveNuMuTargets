...
int main(const int argc, const char** argv)
{
  ...
  for(const auto& prefix: crossSectionPrefixes)
  {
    try
    {
      ...
      auto effNum = util::GetIngredient<PlotUtils::MnvH1D>(*mcFile, "efficiency_numerator_Target7", prefix);
      auto effDenom = util::GetIngredient<PlotUtils::MnvH1D>(*mcFile, "efficiency_denominator_Target7", prefix);


      auto nNucleons = util::GetIngredient<TParameter<double>>(*mcFile, "target7_fiducial_nucleons", prefix); //Dan: Use the same truth fiducial volume for all extractions.  The acceptance correction corrects data back to this fiducial even if the reco fiducial cut is different.
      ...
      backgrounds.push_back(util::GetIngredient<PlotUtils::MnvH1D>(*mcFile, "tgtTarget7_Wrong_Material_Bkg", prefix));
      ...
      //if (false)
      {
        ...
        migration->Add( util::GetIngredient<PlotUtils::MnvH2D>(*mcFile, (std::string("migration_Target")+std::to_string(tgtnum)), prefix));
        effNum->Add( util::GetIngredient<PlotUtils::MnvH1D>(*mcFile, (std::string("efficiency_numerator_Target")+std::to_string(tgtnum)), prefix));
        effDenom->Add( util::GetIngredient<PlotUtils::MnvH1D>(*mcFile, (std::string("efficiency_denominator_Target")+std::to_string(tgtnum)), prefix));

        nNucleons = util::GetIngredient<TParameter<double>>(*mcFile, (std::string("target")+std::to_string(tgtnum)+std::string("_fiducial_nucleons")), prefix); //Dan: Use the same truth fiducial volume for all extractions.  The acceptance correction corrects data back to this fiducial even if the reco fiducial cut is different.

        nNucleonsVal += nNucleons->GetVal();
        backgrounds.push_back(util::GetIngredient<PlotUtils::MnvH1D>(*mcFile, (std::string("tgtTarget")+std::to_string(tgtnum)+std::string("_Wrong_Material_Bkg")), prefix));
        backgrounds.push_back(util::GetIngredient<PlotUtils::MnvH1D>(*mcFile, (std::string("tgtTarget")+std::to_string(tgtnum)+std::string("_Wrong_Sign_Bkg")), prefix));
        backgrounds.push_back(util::GetIngredient<PlotUtils::MnvH1D>(*mcFile, (std::string("tgtTarget")+std::to_string(tgtnum)+std::string("_NC_Bkg")), prefix));
