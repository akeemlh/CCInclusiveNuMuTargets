  ...
  if (files) {
      ...
      while ((file=(TSystemFile*)next())) {
          ...
          if (!file->IsDirectory()) {
              ...
              if(fname.EndsWith("combinedData.root")) //Just this once while I'm using combined for testing
              {
                TFile *data = TFile::Open(dir.GetName()+fname);
              ...
              else if(fname.EndsWith("combinedMC.root")) //Just this once while I'm using combined for testings
              {
                TFile *mc = TFile::Open(dir.GetName()+fname);
...
int main(const int argc, const char** argv)
{
  ...
  std::string dataFilePath = std::string(argv[2])+"/combinedData.root";
  auto dataFile = TFile::Open(dataFilePath.c_str(), "READ");

  if(!dataFile)
  {
    std::cerr << "Failed to open combined data file " << argv[2] << ". Did you run madd first?\n";
    return 2;
  }
  std::string mcFilePath = std::string(argv[2])+"/combinedMC.root";
  auto mcFile = TFile::Open(mcFilePath.c_str(), "READ");
  if(!mcFile)
  {
    std::cerr << "Failed to open combined MC file " << argv[3] << ". Did you run madd first? \n";
    return 3;
  };
  std::cout<<"Here-11\n";
  ...
  for(const auto& prefix: crossSectionPrefixes)
  {
    ...
    //for(const auto& TgtCode: util::TgtCodeLabelsNuke) 
    {
      ...
      {
        std::cout<<"Here1\n";
        auto flux = util::GetIngredient<PlotUtils::MnvH1D>(*mcFile, "target"+TgtCode.second+"_reweightedflux_integrated", prefix);
        ...
        */
        ...
        auto nNucleons = util::GetIngredient<TParameter<double>>(*mcFile, "target"+TgtCode.second+"_fiducial_nucleons", prefix); //Dan: Use the same truth fiducial volume for all extractions.  The acceptance correction corrects data back to this fiducial even if the reco fiducial cut is different.
